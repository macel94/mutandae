...
- Web-layer tests assert the modal, the protocol section, the build link, the
  explicit `tenant `/`account `/`project ` footer scopes, and the absence of
  Alpine attributes.
...
- Live checks after rollout: `https://mutandae.com/api/v1/configuration`
  reports `vault:cluster` and `vault:azure-entra` features; provisioning an
  identity in each cloud shows both the native vault line and the μVault
  (cluster) line in the inventory; the audit modal lists the delivered /
  mirrored / used events; the footer links the build commit.
